/**
 * sections/Hero.jsx
 * Główna sekcja powitalna z interaktywnym tekstem.
 */

import React, { useState, useEffect } from "react";
import { motion } from "framer-motion";
import { heroRotatingWords } from "../../data/content";
...
export function Hero() {
  ...
  // Logika maszyny do pisania (Typewriter Effect)
  useEffect(() => {
    if (typeof window === "undefined") return;
    const reduceMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;

    if (reduceMotion) {
      setTypedText(heroRotatingWords[0]);
      return;
    }

    const currentWord = heroRotatingWords[typedWordIndex];
    const isWordComplete = typedText === currentWord;
    const isWordCleared = typedText === "";

    const typingDelay = isDeleting ? 38 : 72;
    const pauseDelay = isWordComplete ? 1600 : isWordCleared && isDeleting ? 120 : typingDelay;
    ...
    const timeout = setTimeout(() => {
      ...
      if (isDeleting && isWordCleared) {
        setIsDeleting(false);
        setTypedWordIndex((prev) => (prev + 1) % heroRotatingWords.length);
      }
    }, pauseDelay);

    return () => clearTimeout(timeout);
  }, [typedText, typedWordIndex, isDeleting]);
  ...
  return (
    ...
    >
      ...
      {/* Warstwy gradientowe i tekstury dla głębi */}
      ...
      {/* Wielki dekoracyjny inicjał w tle */}
      ...
      <div className="absolute bottom-6 left-1/2 flex -translate-x-1/2 flex-col items-center gap-2 font-mono text-[11px] uppercase tracking-[0.18em] text-zinc-500">
        <span>Scroll</span>
        <span className="relative h-10 w-px overflow-hidden bg-gradient-to-b from-transparent to-zinc-500/60">
          <span
